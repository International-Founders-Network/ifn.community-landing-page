...
export function ValueProps() {
    ...
    return (
        <section className="py-24 bg-paper" id="mentorship">
            <Container>
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <p className="inline-flex items-center rounded-full bg-band px-4 py-2 text-xs font-bold uppercase tracking-[0.1em] text-muted mb-6">
                        Austin, Texas
                    </p>
                    <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-ink mb-5">
                ...
                >
                    {features.map((feature) => (
                        <motion.div
                            key={feature.title}
                            variants={item}
                            className="group flex gap-6 p-6 sm:p-8 rounded-2xl bg-paper border border-rule transition-[background-color,border-color,box-shadow] duration-300 hover:border-edge hover:shadow-lg"
                        >
                            <div
                                className="shrink-0 w-14 h-14 rounded-xl bg-band border border-rule flex items-center justify-center transition-colors duration-300 group-hover:bg-ink group-hover:border-ink"
                                aria-hidden="true"
                            >
                                <feature.icon
                                    className="w-6 h-6 text-ink transition-colors duration-300 group-hover:text-paper"
                                    strokeWidth={1.5}
                                />
                            </div>
                            <div>
