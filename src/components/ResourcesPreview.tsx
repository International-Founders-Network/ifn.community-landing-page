...
export function ResourcesPreview() {
    ...
    return (
        ...
        <section id="resources" className="py-24 bg-paper relative overflow-hidden">
            <Container>
                {/* Header */}
                    ...
                    {segments.map((segment) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex-none inline-flex items-center min-h-11 px-5 rounded-full font-semibold border text-sm sm:text-base transition-colors duration-300 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-offset-2 focus-visible:ring-offset-paper ${isActive
                                    ? 'bg-ink text-paper border-ink'
                                    : 'bg-paper text-muted hover:bg-band border-rule hover:border-edge'
                                    }`}
                            >
                                {segment.name}
                            </button>
                ...
                <div className="relative max-w-5xl mx-auto mb-16 min-h-[36rem] md:min-h-[15rem]">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            {/* Connector, drawn once when the path changes. --ink, not a
                                fifth brand colour: structure is monochrome here. */}
                            <div aria-hidden="true" className="hidden md:block absolute top-[11px] left-0 right-0 h-[2px] bg-band z-0">
                                <motion.div
                            ...
                            <div aria-hidden="true" className="block md:hidden absolute left-[11px] top-0 bottom-0 w-[2px] bg-band z-0">
                                <motion.div
                                ...
                                return (
                                    ...
                                    >
                                        <div aria-hidden="true" className="flex-shrink-0">
                                            <div
                                                className={`w-6 h-6 rounded-full bg-paper border flex items-center justify-center transition-colors duration-300 ${isHovered ? 'border-accent' : 'border-rule'
                                                    }`}
                                            >
                                                <div
                                                    className={`w-2.5 h-2.5 rounded-full transition-colors duration-300 ${isHovered ? 'bg-accent' : 'bg-rule'
                                                        }`}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-col md:items-center md:mt-4 w-full">
                                            <h3 className="font-bold text-ink mb-1 leading-tight">{stage.name}</h3>
                                            <p className="text-sm text-muted leading-snug">{stage.description}</p>
                                        </div>
                                    </motion.div>
                                );
                            })}
                ...
                {examples.length > 0 && (
                    <div className="max-w-4xl mx-auto mb-12">
                        <h3 className="text-xs font-bold uppercase tracking-[0.1em] text-muted text-center mb-5">
                        ...
                        <ul className="grid gap-4 sm:grid-cols-3">
                            {examples.map((resource) => (
                                ...
                                >
                                    ...
                                    <p className="mt-2 font-semibold text-ink leading-snug">{resource.title}</p>
                                </li>
                            ))}
                        </ul>
                    </div>
                ...
                <ul className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-sm font-medium text-muted mb-12">
                ...
                {/* CTA. A link, not a button, because it navigates, and a <button>
                    nested inside an <a> is invalid HTML. The shadow keeps its size
                    utility but no longer names a colour: plan section 4.4 bans every
                    resting shadow outright, and Phase 3 removes the last one. Until
                    then the size utility falls back to Tailwind's own neutral, which
                    cannot repaint the focus ring the way a colour utility could. */}
                <div className="text-center">
                    ...
                    <p className="mt-6 text-sm text-muted">
                        ...
                        <a
                            ...
                            className="font-semibold text-ink underline underline-offset-4 decoration-rule hover:decoration-ink transition-colors rounded-sm focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-offset-2 focus-visible:ring-offset-paper"
                        >
