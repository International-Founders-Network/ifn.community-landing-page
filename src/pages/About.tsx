...
export function About() {
    ...
    return (
        <div className="pt-24 pb-20">
            {/* Hero */}
            <section className="bg-band py-24 mb-24 relative overflow-hidden">
            ...
            <Container className="mb-32">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
                    <div>
                        <h2 className="text-3xl font-bold text-ink mb-6 tracking-tight">Why IFN exists</h2>
                    ...
                    <div className="bg-band border border-rule rounded-2xl p-8 sm:p-10">
                        <h3 className="text-xl font-bold text-ink mb-2">What a meetup actually is</h3>
                        ...
                        <dl className="divide-y divide-rule">
                            {facts.map((fact) => (
                                <div key={fact.label} className="py-5 first:pt-0 last:pb-0">
                                    <dt className="text-xs font-bold uppercase tracking-[0.1em] text-muted mb-2">
                                        {fact.label}
                                    </dt>
                                    <dd>
                                        <span className="block text-lg font-bold text-ink mb-1">{fact.value}</span>
                                        <span className="block text-muted leading-relaxed">{fact.detail}</span>
                                    </dd>
                                </div>
                            ))}
                        </dl>
                        <a
                            ...
                            className="inline-flex items-center mt-8 text-base font-bold text-ink underline underline-offset-4 decoration-rule hover:decoration-ink transition-colors rounded-sm focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-offset-2 focus-visible:ring-offset-paper"
                        >
            ...
            {/* How we work: recessed band. This was a dark inset slab before the token sweep. */}
            <section className="bg-band py-32 rounded-3xl mx-4 sm:mx-8 mb-32 overflow-hidden relative">
                <div className="absolute top-0 right-0 w-96 h-96 bg-ink opacity-5 rounded-full blur-[100px] -mr-48 -mt-48" />
                <div className="absolute bottom-0 left-0 w-96 h-96 bg-accent opacity-10 rounded-full blur-[100px] -ml-48 -mb-48" />

                <Container>
                    <div className="text-center mb-20">
                        <h2 className="text-4xl font-bold text-ink mb-6 tracking-tight">How we work</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {values.map((v, i) => (
                            <motion.div
                                ...
                                viewport={{ once: true }}
                                className="bg-paper border border-rule p-8 rounded-2xl hover:border-edge transition-colors"
                            >
                                <div className="bg-band text-ink p-3 rounded-lg inline-flex mb-6">
                                    {v.icon}
                                </div>
                                <h3 className="text-xl font-bold text-ink mb-4">{v.title}</h3>
