# membership-billing Specification

## Purpose
Subscription checkout, Stripe webhook ingestion, and the server-side record of
who is a paying member. Stripe remains the system of record for money; the
`memberships` table exists so IFN can answer "is this person a member, and when
does their access lapse?" without opening the Stripe dashboard.

Created 2026-08-28 by `add-membership-subscription-billing`.
