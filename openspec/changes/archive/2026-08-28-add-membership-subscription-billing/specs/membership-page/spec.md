...
**Reason**: Replaced by Stripe Checkout. `/contact` and `netlify/functions/contact.ts`
are untouched and remain the fallback whenever checkout fails, so the hand-sold
flow is not dismantled — only the CTA's default destination changes.
